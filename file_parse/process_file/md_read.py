...
async def query_md_file_encoding(file_path) -> Optional[Any]:
    """获取Markdown文件编码"""
    try:
...
async def read_md_in_chunks(file_path, encoding, chunk_size=CHUNK_SIZE) -> AsyncIterator[str]:
    """
    迭代器：分块读取大的Markdown文件
    :param encoding: 文件编码
...
async def convert_md_file_encoding(file_path, to_encoding: str) -> None:
    """ Markdown编码转换 """
    file_encode = await query_md_file_encoding(file_path)
...
@cost_time
async def read_md_file(file_path) -> Dict[str, object]:
    """
    读取Markdown文件
    :return: 文件内容字符串
    """
    file_encode = await query_md_file_encoding(file_path)
    chunks = []
    async for chunk in read_md_in_chunks(file_path, encoding=file_encode):
        chunks.append(chunk)
